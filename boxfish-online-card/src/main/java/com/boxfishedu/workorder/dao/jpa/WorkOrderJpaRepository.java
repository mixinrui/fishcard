...
public interface WorkOrderJpaRepository extends JpaRepository<WorkOrder, Long> {
    ...
    @Query(value = "select wo from WorkOrder wo where wo.studentId=?1 and wo.startTime>?2 and wo.skuId=?3 and wo.isFreeze=?4  and (wo.classType not in(?6) or wo.classType is null)  ")
