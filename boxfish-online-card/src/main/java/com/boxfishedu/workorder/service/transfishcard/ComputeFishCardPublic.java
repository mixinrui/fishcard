 ...
 */
@Component
public class ComputeFishCardPublic{
...
//    RoleEnum
    ...
    private void sendMessage(Map<String,SmallClassLog> disSmallClassLog,Map<Long,Integer>   smallClassesForCHMap,Map<Long,String>   smallClassesCourseIDMap){
        ...
        disSmallClassLog.forEach((key,smallClassLog)->{
                    ...
                    if ( checkISForeign(smallClassesForCHMap.get(smallClassLog.getSmallClassId()))) { //判断外教
                        studentSysParam.setChannel_type(ChannelTypeEnum.COURSE_FOREIGN_OPEN);
                    }else {
                        studentSysParam.setChannel_type(ChannelTypeEnum.COURSE_CHINESE_OPEN);
                    }
    ...
    public List<WorkOrder> getMyListWorks(){
        return  workOrderJpaRepository.findByComputeSendDatasPUBLIC(getBeginDate(),getBaseNowDate(), ClassTypeEnum.PUBLIC.name());
    }
