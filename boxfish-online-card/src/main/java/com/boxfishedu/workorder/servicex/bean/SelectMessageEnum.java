...
public enum SelectMessageEnum {
    ...
    NORM_CLASS_BEFORE_1_1LESSON(Weeks.LESS_AND_EQUALS_THAN_1_1LESSON, ClassTypeEnum.SMALL, "您已选择以下时间段课程，最快后天可上课"),  //小班课  一节课的情况
    ...
    public enum Weeks {
        LESS_AND_EQUALS_THAN_1_1LESSON(-1),LESS_AND_EQUALS_THAN_1(1), GREATER_THAN_1(2), DEFAULT(0);
        ...
        public static Weeks resolve(int week) {
            if(-1 == week)
                return LESS_AND_EQUALS_THAN_1_1LESSON;
    ...
    public static SelectMessageEnum resolve(ClassTypeEnum classType, Weeks weeks,Integer class_num) {
        // 小班课
        if(classType == ClassTypeEnum.SMALL) {
            if(weeks == Weeks.LESS_AND_EQUALS_THAN_1) {
                if(!Objects.isNull(class_num) && class_num.intValue()==1){
                    return NORM_CLASS_BEFORE_1_1LESSON;  //小班课如果是一节 返回 提示
                }
